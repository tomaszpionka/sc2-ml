...
| **Severity** | major |
| **Likelihood** | low |
| **Status (PR #208 T12, 2026-05-05)** | **MITIGATED-NARROWED.** Step 01_03_05 executed end-to-end in PR #208 (`phase01/sc2egset-tracker-events-semantic-validation`). GATE-14A6 closed to `narrowed`. The planned Phase 02 tracker-derived subset (12 of 15 candidate feature families) is mitigated and ready under each row's recorded `eligibility_scope`. Three blocked families remain OPEN with explicit blocking reasons (see below). RISK-21 is **NOT fully resolved**. Tracker-derived features remain **never pre-game** (Amendment 2 / Invariant I3); the full `tracker_events_raw` semantic scope is **not fully closed**. |
| **Affected thesis sections** | `thesis/chapters/04_data_and_methodology.md` §4.3.2 / §4.4 (Phase 02 in-game features — not yet drafted); applies to all chapter prose that references SC2 in-game-snapshot tracker-derived features. |
| **Evidence source (post-mitigation)** | Step 01_03_05 artifacts under `src/rts_predict/games/sc2/datasets/sc2egset/reports/artifacts/01_exploration/03_profiling/`: `01_03_05_tracker_events_semantic_validation.json` (V1..V8 verdicts), `01_03_05_tracker_events_semantic_validation.md`, and `tracker_events_feature_eligibility.csv` (15 rows; per-prediction-setting columns per Amendment 2). Generated 2026-05-05 in PR #208 T11. Underlying data: `tracker_events_raw` (62,003,411 rows), `replays_meta_raw` (22,390 rows), `replay_players_raw` (44,817 rows). Original evidence (cross_dataset_comparability_matrix.md §1) is preserved for historical context but is now superseded by the Step 01_03_05 artifacts for the planned-yes subset. |
| **Mitigation applied (PR #208)** | Step 01_03_05 V1..V8 validated semantic eligibility for 12 candidate feature families (5 `eligible_for_phase02_now` + 7 `eligible_with_caveat`). PlayerStats cumulative-economy concern resolved by Q3 strict rule: `*Lost` / `*Killed` / `*FriendlyFire` / `*Used` keys are blocked because s2protocol does NOT confirm cumulative semantics (`SOURCE_CONFIRMS_CUMULATIVE` set is empty by design). PlayerStats *snapshot* fields (e.g., `MineralsCurrent`, `WorkersActiveCount`, `FoodUsed`/`FoodMade` with fixed-point caveat) classified `safe_snapshot` and exposed for in-game-snapshot use. |
| **Residual uncertainty (post-mitigation)** | Three tracker candidate families remain OPEN and require additional validation before they can enter Phase 02 scope: (1) **PlayerStats cumulative-economy fields** — require source-confirmed cumulative semantics (Blizzard authoritative source or replay-engine corroboration) or an alternative derivation; (2) **UnitOwnerChange dynamic-ownership / mind-control features** — sparse coverage (absent in 2016, ~25% of replays in 8 of 9 years per V4) prevents broad use without a tournament-meta-aware redesign; (3) **UnitPositions coordinate / army-centroid features** — require both (a) a validated packed-items decoder (`(delta_index, x, y)` triplet walk + UnitBorn-lineage owner attribution) and (b) coordinate units/origin source confirmation per Amendment 5 (V6 `source_confirmed_units = False` AND `source_confirmed_origin = False`). Additionally: SC2EGSet decoder scaling for `scoreValueFoodUsed` / `scoreValueFoodMade` (divide-by-4096 vs pre-scaled per s2protocol README) is **not resolved**; Phase 02 must resolve scaling discipline before consuming those two fields. |
| **Wording recommendation (post-mitigation)** | "SC2 in-game telemetry features derived from `tracker_events_raw` are validated for use in an in-game-snapshot setting under Step 01_03_05 (GATE-14A6 closed to `narrowed`, 2026-05-05). The validated subset comprises 12 candidate feature families with explicit per-family eligibility scopes recorded in `tracker_events_feature_eligibility.csv`. Three additional candidate families (PlayerStats cumulative economy, UnitOwnerChange dynamic ownership, UnitPositions coordinate features) remain `blocked_until_additional_validation` and are excluded from initial Phase 02 scope. Tracker-derived features are never pre-game (Amendment 2 / Invariant I3); the cutoff rule is `event.loop <= cutoff_loop` (game loops). Cumulative-economy semantics for PlayerStats `*Lost` / `*Killed` / `*FriendlyFire` / `*Used` keys are not confirmed by the s2protocol decoder; cumulative-style features are blocked under the Q3 strict rule." Cite `tracker_events_feature_eligibility.csv` as the authoritative per-family table. |
| **Future validation route** | Promote `full_tracker_scope_closed_predicate_satisfied` from `false` to `true` by: (a) source-confirming PlayerStats cumulative semantics; (b) implementing+validating the UnitPositions packed-items decoder; (c) source-confirming SC2 tracker coordinate units and origin; (d) either redesigning UnitOwnerChange features for sparse coverage or accepting their narrow availability. These require additional dedicated Steps; out of scope for PR #208. |
| **Downstream task responsible** | RESOLVED for the planned subset by PR #208 (T01..T13). For blocked families: future dedicated Steps (not currently scheduled). T11 of any future thesis-sync PR for §4.3.2 / §4.4 must source wording from `tracker_events_feature_eligibility.csv` and `phase02_readiness_hardening.md` §14A.6 post-validation subsection. |
| **Blocking before Chapter 4 rewrite** | no (the post-mitigation wording recommendation above unblocks Chapter 4 §4.3.2 / §4.4 prose for the planned subset; blocked families must be cited explicitly as remaining out of scope) |
