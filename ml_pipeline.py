...
def perform_feature_engineering(df):
    logger.info("Rozpoczynam Zaawansowany Feature Engineering (z poprawką na Data Leakage)...")
    ...
    for player_prefix in ['p1', 'p2']:
        ...
        df[f'{player_prefix}_hist_mean_apm'] = np.where(games_played > 0, hist_apm_sum / games_played, global_mean_apm)
        df[f'{player_prefix}_hist_mean_sq'] = np.where(games_played > 0, hist_sq_sum / games_played, global_mean_sq)
        
        # Supply Block (Jak bardzo gracz "zapomina" w przeszłości)
        hist_supply_sum = get_historical_sum(f'{player_prefix}_id', f'{player_prefix}_supply_capped_pct')
        df[f'{player_prefix}_hist_mean_supply_block'] = np.where(games_played > 0, hist_supply_sum / games_played, global_mean_supply)
        
        # Preferencje długości gry (Czy gracz to "Rusznikarz" czy "Żółw")
        hist_loops_sum = get_historical_sum(f'{player_prefix}_id', 'game_loops')
        df[f'{player_prefix}_hist_mean_game_length'] = np.where(games_played > 0, hist_loops_sum / games_played, global_mean_loops)
    ...
    df['diff_hist_supply_block'] = df['p1_hist_mean_supply_block'] - df['p2_hist_mean_supply_block']
    df['diff_hist_game_length'] = df['p1_hist_mean_game_length'] - df['p2_hist_mean_game_length']
    ...
    cols_to_drop = [
        ...
        'p1_apm', 'p2_apm', 'p1_sq', 'p2_sq', 
        'p1_supply_capped_pct', 'p2_supply_capped_pct', # <- WYCIEK DANYCH
        'game_loops', 'data_build', 'p2_target_perspective', # <- WYCIEK DANYCH
        'map_size_x', 'map_size_y' # Wyrzucamy bo mamy już map_area
    ]
    ...
    logger.info(f"Feature Engineering zakończony. Zbudowano {features_df.shape[1]} czystych cech.")
    return features_df
