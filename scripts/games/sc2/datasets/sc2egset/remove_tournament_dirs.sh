#!/usr/bin/env bash
# Removes tournament subdirectories ONLY when ALL three guards pass:
#   1. A corresponding <tournament>.zip exists beside the directory (at raw/ level).
#   2. The zip is non-empty (>0 bytes).
#   3. The total file count in the zip matches the file count in the directory.
#
# Run AFTER rezip_tournaments.sh. Any directory that fails a guard is left untouched.
...
for tournament_dir in "$TARGET"/20*; do
    ...
    # Guard 1: zip must exist at raw/ level
    if [[ ! -f "$zip_file" ]]; then
    ...
    dir_count=$(find "$tournament_dir" -type f | wc -l | tr -d ' ')
    ...
    if rm -rf "$tournament_dir"; then
        echo "    Removed."
        ((removed++))
    else
        echo "    ERROR: rm -rf failed for $tournament_dir"
        ((skipped_mismatch++))
    fi
