    ...
    for key, val in primary_block.items():
        ...
        for metric_key in ["icc_anova_observed_scale", "icc_lpm_observed_scale"]:
            ...
            if icc_point is None:
                continue
            # v1.0.5: CI bounds live in dedicated columns on the same row.
            ci_lo = mv.get("ci_lo")
            ci_hi = mv.get("ci_hi")
            icc_rows.append({
                ...
                "metric_ci_low": round(float(ci_lo), 4) if ci_lo is not None else None,
                "metric_ci_high": round(float(ci_hi), 4) if ci_hi is not None else None,
...
if dgp_ref_path.exists():
    ...
    for _, row in df_dgp_ref.iterrows():
        dgp_rows.append({
            ...
            "metric_value": 0.0,
            "metric_ci_low": None,
            "metric_ci_high": None,            "reference_window_id": "2022-Q3-patch66692",
            "cohort_threshold": 0,
...
# v1.0.5: 11-column schema (CI columns added).
SCHEMA_COLS = ["dataset_tag", "quarter", "feature_name", "metric_name",
               "metric_value", "metric_ci_low", "metric_ci_high",
               "reference_window_id", "cohort_threshold",
               "sample_size", "notes"]

df_p06 = pd.concat(all_frames, ignore_index=True)
# Ensure schema columns exist (PSI CSVs from pre-v1.0.5 lack metric_ci_*; inject None).
...
_invalid = set(df_p06["metric_name"].dropna().unique()) - _VALID_METRIC_NAMES
assert not _invalid, (
    f"v1.0.5 enum violation: metric_name values not in closed set: {_invalid}"
)
...
# %%
# Schema validation
# v1.0.5: schema is 11 columns (added metric_ci_low, metric_ci_high).
validation = {
    "schema_version": "1.0.5",
    "column_count_ok": len(df_p06.columns) == 11,
    "column_count": len(df_p06.columns),
    "columns_match_spec": df_p06.columns.tolist() == SCHEMA_COLS,
    ...
    "metric_name_in_closed_enum": (set(df_p06["metric_name"].dropna().unique()) <= _VALID_METRIC_NAMES),
    "n_rows": len(df_p06),
...
if not validation["column_count_ok"]:
    print(f"FAILED: expected 11 columns (v1.0.5), got {validation['column_count']}")
elif not validation["metric_name_in_closed_enum"]:
    print(f"FAILED: metric_name values outside v1.0.5 closed enum")
elif not validation["n_rows_above_64"]:
    print(f"WARNING: only {len(df_p06)} rows (< 64 expected minimum per m5)")
else:
    print("Schema validation PASSED (v1.0.5)")
...
- Column count: {validation['column_count']} (expected 11, v1.0.5): {'OK' if validation['column_count_ok'] else 'FAIL'}
- Columns match spec order: {validation.get('columns_match_spec')}
- metric_name ⊆ closed enum: {validation.get('metric_name_in_closed_enum')}
