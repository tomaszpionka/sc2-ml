...
for source, cols in schemas.items():
    md_lines.append(f"## {source} ({len(cols)} columns)")
    md_lines.append("")
    md_lines.append("```sql")
    md_lines.append(DESCRIBE_SQL.get(source, f"DESCRIBE {source}"))
    md_lines.append("```")
    md_lines.append("")
    md_lines.append("| column_name | column_type | null | key | default | extra |")
    md_lines.append("|---|---|---|---|---|---|")
    for col in cols:
        md_lines.append(
...
md_lines.append("---")
md_lines.append("")
md_lines.append("*Generated by notebook 01_02_03_raw_schema_describe.py*")

md_path = artifacts_dir / "01_02_03_raw_schema_describe.md"
md_path.write_text("\n".join(md_lines) + "\n")
