...
- **`old_rating` CONDITIONAL_PRE_GAME classification:** Classified CONDITIONAL_PRE_GAME per Phase 01 01_04 classification discipline (docs/PHASES.md §Phase 01). **Condition:** `old_rating` is PRE-GAME iff `leaderboard = 'random_map'` AND (`time_since_prior_match_days < 7` OR `time_since_prior_match_days IS NULL`) — first-match NULL handled via the inner disjunction; the scope predicate `leaderboard = 'random_map'` gates both the <7d cutoff AND the NULL fallback (per SQL AND-over-OR precedence the explicit parentheses around the disjunction are load-bearing: Phase 02 consumers MUST copy the parentheses verbatim or rewrite to an equivalent form like `leaderboard = 'random_map' AND COALESCE(time_since_prior_match_days, 0) < 7`). The scope predicate `leaderboard = 'random_map'` applies because not all leaderboards clear 0.90 at the chosen N*=7 cutoff: `team_random_map`=0.868, `co_random_map`=0.868, `co_team_random_map`=0.795 (all < 0.90 at <7d). N*=7 is the empirically-selected threshold: the largest N ∈ {1, 2, 3, 7} days where pooled agreement on `random_map` clears the WP-4 0.90 stratum gate (01_04_07 §4): N=1→0.9440, N=2→0.9393, N=3→0.9369, N=7→0.9322 — all pass 0.90, so N*=7 (largest). **Empirical grounding:** 01_04_06 (leaderboard-partitioned consecutive-match test) + 01_04_07 (threshold-selection + 4×4 leaderboard × time-gap stratification). **Structural supplement:** `matches_1v1_clean.yaml:excluded_columns` excludes `new_rating`/`p*_new_rating` as POST-GAME at the clean-view level. **Derived column anchor:** `time_since_prior_match_days` added to `player_history_all` VIEW (15-col schema per 01_04_07 amendment). **Phase 02 implications:** rolling features using `old_rating` should apply the condition as a filter or use dual feature paths (within-scope vs cross-session). (01_03_01 + 01_04_06 + 01_04_07)
