...
and subdirectories at two levels: the top-level tournament directories (level 1)
and each tournament's `_data/` subdirectory (level 2). Filename patterns were
...
# Level 1: tournament directories
meta_result = inventory_directory(RAW_DIR)

# Level 2: each tournament's _data/ subdir
...
- 70 top-level tournament directories under `raw/`
- 3 files at `raw/` root: `README.md`, `README.sc2egset.md`, `SC2EGSet_datasheet.pdf`
- 431 metadata files distributed across tournament directories (`.zip`, `.log`, `.json` patterns)
- Each tournament directory contains one `map_foreign_to_english_mapping.json` (70 total)
...
- Replays per tournament: min 30, max 1,296, median 260.5
- 0 tournaments with a missing `_data/` subdirectory
...
- Whether any of the 70 tournament directories should be excluded from analysis
...
- The `{hash}.SC2Replay.json` filename pattern does not embed dates; dates are
  encoded only in tournament directory names, not individual filenames. Whether
  individual files carry internal timestamps must be established at Step 01_01_02
  (content/schema profiling).
- The per-tournament replay count range (30 to 1,296) is wide; whether smaller
  tournaments (e.g., those with < 50 files) are complete acquisitions or partial
  extractions cannot be determined from filenames alone.
- The `processed_failed.log` files present in every tournament directory indicate
