...
| `map` | `player_history_all.metadata_mapName` (joined to the focal row through the dataset-specific native keys and temporal anchors prescribed by CROSS-02-00-v3.0.1 §3.2 / §5; do not assume generic `player_history_all.match_id` or `player_history_all.player_id` columns) | match_pair focal row | `started_at` | none | 188 distinct map names in sc2egset; encoder fit per `dataset_tag` partition. |
...
| `map` | `player_history_all.map` joined to the focal row through the dataset-specific native keys and temporal anchors prescribed by CROSS-02-00-v3.0.1 §3.2 / §5; do not assume generic `player_history_all.match_id` or `player_history_all.player_id` columns | match_pair focal row | `started_at` | none | 77+ distinct map names in aoestats 1v1 Random Map scope (source label `leaderboard='random_map'`; queue semantics unverified); encoder fit per `dataset_tag` partition; vocabulary disjoint from sc2egset. |
