...
- **aoestats NOTE 2 — "43-day post-patch gap" figure no artifact provenance.** The 43-day post-patch gap referenced in aoestats temporal analysis lacks a committed artifact or SQL query establishing its provenance. The figure appears in the research log without a corresponding notebook or artifact output. Severity: LOW (documentation gap; no methodology impact). Scheduled closure: WP-5 (artifact provenance step in Phase 02 setup).
...
| aoestats | WARNING 2 | MEDIUM | `old_rating` PRE-GAME classification unverified | WP-4 (scheduled) | OPEN — SCHEDULED |
| aoestats | NOTE 1 | LOW | `player_history_all` interface docs gap | WP-1 PR #198 §3.2 | CLOSED |
| aoestats | NOTE 2 | LOW | "43-day post-patch gap" figure no artifact provenance | WP-5 (scheduled) | OPEN — SCHEDULED |
